...
function findRallyListTable() {
  return findBestMatchingTable({
    selector: 'table',
    includeTfoot: false,
    minScore: 1,
    score: ({ rows }) => {
      if (rows.length < 2) return 0;
      return scoreRallyListTable(rows);
    },
  });
}
...
function insertRallySearchBar(table, totalRows) {
  ...
  bar.innerHTML = `
    <input
      type="search"
      class="rsf-plugin-search-input"
      placeholder="Search rallies..."
      aria-label="Search rallies"
    />
    <label class="rsf-plugin-search-check">
