(() => {
  ...
  function parseTimeToSeconds(timeStr) {
    ...
    const s2 = timeStr.trim();
    if (!s2 || s2 === "-" || s2.toLowerCase() === "n/a") return null;
  ...
  function parseDiffToSeconds(str) {
    ...
    const s2 = str.trim();
    if (!s2 || s2.includes("-")) return null;
    if (!s2.includes(":")) {
      if (!/^\d+(?:[.,]\d+)?$/.test(s2)) return null;
      const seconds = Number(s2.replace(",", "."));
      return Number.isFinite(seconds) ? seconds : null;
    }
    return parseTimeToSeconds(s2);
  }
  function parseKm(lenStr) {
    if (typeof lenStr !== "string") return null;
    const s2 = lenStr.trim();
    if (!s2) return null;
    const match2 = s2.match(/^(\d+(?:[.,]\d+)?)\s*km$/i);
    if (!match2) return null;
    const km = Number(match2[1].replace(",", "."));
  ...
  function addDrivenStage(stats, diff2, km) {
    if (!stats || !Number.isFinite(diff2) || !Number.isFinite(km) || km <= 0) {
      return stats;
    }
    stats.drivenCount += 1;
    stats.drivenKm += km;
    stats.diffs.push(diff2);
    return stats;
  }
  ...
  function insertDiffDataCell(row, cols, stats) {
    ...
    if (km && km > 0 && prSec !== null && wrSec !== null) {
      const diff2 = (prSec - wrSec) / km;
      setSecondsPerKmCell(td, diff2);
      cells[cols.wrIdx].insertAdjacentElement("afterend", td);
      stats = addDrivenStage(stats, diff2, km);
      return "ok";
    }
  ...
  function parseLegRange(text) {
    ...
    if (!match2) return null;
    const start = parseBudapestDateTime(match2[1]);
    const end = parseBudapestDateTime(match2[2]);
  ...
  function extractTotalKmFromHtml(html) {
    ...
    const match2 = text.match(/Total\s+Distance\s+Rally[^0-9]*([\d]+(?:[.,]\d+)?)\s*km/i);
    if (!match2) return null;
    const km = Number(match2[1].replace(",", "."));
    return Number.isFinite(km) ? km : null;
  }
  ...
  function insertRallySearchBar(table, totalRows) {
    ...
    bar.innerHTML = `<input type="text" id="rally-search-input" class="rsf-plugin-search-input"
    ...
    const firstElement = Array.from(container.childNodes).find((node) => node.nodeType === Node.ELEMENT_NODE);
  ...
  function addStagesFilter() {
    ...
    bar.innerHTML = SURFACES.map(
      (s2) => `<label class="rsf-plugin-surface-label rsf-plugin-surface-${s2}">
         <input type="checkbox" class="rsf-plugin-surface-cb" value="${s2}" checked>
